...
тогда найдем $y\notin V[P]$, $y\in V[Q]$. Сделаем замену $P[x\coloneqq{}y]$.
...
\begin{definition}[Параллельная $\beta$-редукция]
	...
	\begin{enumerate}
		...
		$P_{1}\bpar P_{2}$
		\item $A=_{\alpha}(\lambda{}x.P)Q$, $B=_{\alpha}P[x\coloneqq{}Q]$ причем $Q$ свободна для подстановки вместо $x$ в $P$ 
	\end{enumerate}
\end{definition}
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item Пусть $P_{1}=_\alpha(\lambda{}y.A)B$, $P_{2}=_\alpha A[y\coloneqq{}B]$ и подстановка возможна. Из предположения индукции получаем, что $A[x \coloneqq Q_1] \bpar A[x \coloneq Q_2]$, $B[x \coloneqq Q_1] \bpar B[x \coloneqq Q_2]$. Значит $P_1[x \coloneqq Q_1]$ с учетом редукций примет вид $(\lambda{}y.A[x \coloneqq Q_2]) B[x \coloneqq Q_2]$. $P_2[x \coloneqq Q_2]$ же примет вид $A[y \coloneqq B][x \coloneqq Q_2]$. Следовательно мы получим, что $P_1[x \coloneqq Q_1] \bpar P_2[x \coloneqq Q_2]$.
	\end{itemize}
\end{proof}
...
\begin{theorem}[Приводится без доказательсвта]
