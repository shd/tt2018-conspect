...
\subsection{Абстрактные типы}	 
	...
	$ Это правило вывода позволяет скрыть реализацию стека, так как если $\textbf{stack}$ --- это тип стека, то $\textbf{stack}[\alpha := \theta]$ --- его конкретная реализация, например ArrayStack, LinkedListStack и подобные \\ \\
 	 $
